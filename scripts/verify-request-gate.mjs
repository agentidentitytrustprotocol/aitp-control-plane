 ...
 * request was stopped by the proxy. This harness stays correct for two reasons
 * that are load-bearing rather than lucky, and both must survive any edit here:
 * `probe()` sends GET, and enroll is POST-only (so the handler answers 405
 * before its own 503 can fire), and `boot()` always supplies a valid 43-char
 * ENROLLMENT_SECRET. Break either and check 10 fails as
 * `/api/registry/enroll: public -> gate-misconfigured`, sending a reader to
 * `proxy.ts` to debug a fault that is actually in the route's own config.
 * The durable fix, if that ever bites, is to scope this classification by route
 * or to give the gate a `code` no handler shares.
 */
function classify(r) {
